...
export default function NovoClientePage() {
  ...
  const [form, setForm] = useState({
    ...
    endereco: '', cidade: '', estado: '', observacoes: '', ativo: true,
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    setError('')
    const { error: err } = await createClient().from('clientes').insert([form])
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <h1 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#111', marginTop: '4px' }}>Novo Cliente</h1>
      </div>
