...
export default function EditarEngenheiroPage() {
  ...
  useEffect(() => {
    createClient().from('engenheiros').select('*').eq('id', id).single()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    setError('')
    const { error: err } = await createClient().from('engenheiros').update(form).eq('id', id)
    if (err) { setError(err.message); setSaving(false); return }
    router.push('/admin/engenheiros/' + id)
  }
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <Link href={'/admin/engenheiros/' + id} style={{ fontSize: '13px', color: '#6b7280', textDecoration: 'none' }}>← Voltar</Link>
