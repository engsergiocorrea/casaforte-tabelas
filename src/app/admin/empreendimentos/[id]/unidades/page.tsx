              ...
              <thead>
                <tr style={{background:"#f9fafb",borderBottom:"1px solid #e5e7eb"}}>
                  {["Unidade","Pavimento","Área","Quartos","Valor","Sinal","Parcelas","Status","Ações"].map((h) => (
                    <th key={h} style={{padding:"10px 12px",textAlign:"left",fontSize:"11px",fontWeight:"600",color:"#6b7280",textTransform:"uppercase",whiteSpace:"nowrap"}}>
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  return (
                    <tr key={u.id} style={{borderBottom:"1px solid #f3f4f6"}}>
                      ...
                      <td style={{padding:"10px 12px",fontWeight:"500",color:"#111"}}>
                        {u.valor_imovel ? `R$ ${Number(u.valor_imovel).toLocaleString("pt-BR",{minimumFractionDigits:2})}` : "—"}
                      </td>
                      <td style={{padding:"10px 12px",color:"#6b7280"}}>
                        {u.valor_sinal ? `R$ ${Number(u.valor_sinal).toLocaleString("pt-BR",{minimumFractionDigits:2})}` : "—"}
                      </td>
                      <td style={{padding:"10px 12px",color:"#6b7280"}}>
                        {u.quantidade_parcelas && u.valor_parcela
                          ? `${u.quantidade_parcelas}x R$ ${Number(u.valor_parcela).toLocaleString("pt-BR",{minimumFractionDigits:2})}`
                          : "—"}
                      </td>
