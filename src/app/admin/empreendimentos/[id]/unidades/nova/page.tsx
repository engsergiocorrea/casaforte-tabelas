...
export default function NovaUnidadePage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({
    unidade: '', bloco: '', pavimento: '', tipo: '', categoria: '',
    area_construida: '', area_privativa_externa: '', area_total: '', area_terreno: '',
    quartos: '', suites: '', banheiros: '', vagas: '',
    posicao: '', valor_imovel: '', percentual_sinal: '10',
    valor_sinal: '', quantidade_parcelas: '60', valor_parcela: '',
    quantidade_intercaladas: '', periodicidade_intercaladas: 'semestrais',
    valor_intercalada: '', valor_chaves: '', percentual_chaves: '20',
    status: 'disponivel', observacoes_publicas: '', destaque: false,
  })
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) {
    ...
    setForm(f => {
      ...
      if ((name === 'valor_imovel' || name === 'percentual_sinal') && updated.valor_imovel && updated.percentual_sinal) {
        updated.valor_sinal = String(Math.round(Number(updated.valor_imovel) * Number(updated.percentual_sinal) / 100))
      }
      ...
      if ((name === 'valor_imovel' || name === 'percentual_sinal' || name === 'percentual_chaves' || name === 'quantidade_parcelas') && updated.valor_imovel) {
        ...
        if (parcelas > 0) updated.valor_parcela = String(Math.round(saldo / parcelas))
      }
      return updated
    })
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const data = {
      ...
      area_privativa_externa: form.area_privativa_externa ? Number(form.area_privativa_externa) : null,
      ...
      percentual_sinal: form.percentual_sinal ? Number(form.percentual_sinal) : null,
      ...
      quantidade_intercaladas: form.quantidade_intercaladas ? Number(form.quantidade_intercaladas) : null,
      valor_intercalada: form.valor_intercalada ? Number(form.valor_intercalada) : null,
      valor_chaves: form.valor_chaves ? Number(form.valor_chaves) : null,
      percentual_chaves: form.percentual_chaves ? Number(form.percentual_chaves) : null,
      posicao: form.posicao || null,
    }
    const { error: err } = await supabase.from('unidades').insert([data])
    if (err) { setError(err.message); setLoading(false); return }
    router.push(`/admin/empreendimentos/${params.id}/unidades`)
  }
  ...
  return (
    <div>
      <div style={{marginBottom:'1.5rem'}}>
        <h1 style={{fontSize:'1.5rem',fontWeight:'700',color:'#111'}}>Nova Unidade</h1>
        <p style={{fontSize:'0.875rem',color:'#6b7280',marginTop:'0.25rem'}}>Os valores são calculados automaticamente</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{background:'white',borderRadius:'12px',border:'1px solid #DDD9D3',padding:'24px',marginBottom:'16px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'14px'}}>
            <div><label style={S.label}>Unidade *</label>{inp('unidade','text','Ex: 101')}</div>
            <div><label style={S.label}>Bloco</label>{inp('bloco','text','Ex: A')}</div>
            <div><label style={S.label}>Pavimento</label>{inp('pavimento','text','Ex: 1º Pavimento')}</div>
            <div><label style={S.label}>Tipo</label>{inp('tipo','text','Ex: Apartamento')}</div>
            ...
            <div>
              ...
              {sel('posicao',[{value:'',label:'Selecione'},{value:'frente_mar',label:'Frente mar'},{value:'lateral',label:'Lateral'},{value:'nascente',label:'Nascente'},{value:'poente',label:'Poente'},{value:'terreo',label:'Térreo'},{value:'rooftop',label:'Rooftop'},{value:'outra',label:'Outra'}])}
            </div>
            <div>
              <label style={S.label}>Status</label>
              {sel('status',[{value:'disponivel',label:'Disponível'},{value:'reservada',label:'Reservada'},{value:'vendida',label:'Vendida'},{value:'bloqueada',label:'Bloqueada'},{value:'indisponivel',label:'Indisponível'}])}
            </div>
          </div>
        </div>
        ...
        <div style={{background:'white',borderRadius:'12px',border:'1px solid #DDD9D3',padding:'24px',marginBottom:'16px'}}>
          <h2 style={{fontSize:'15px',fontWeight:'600',color:'#111',marginBottom:'4px'}}>Valores financeiros</h2>
          <p style={{fontSize:'12px',color:'#9ca3af',marginBottom:'16px'}}>Sinal e parcela são calculados automaticamente ao preencher o valor do imóvel</p>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'14px'}}>
            <div><label style={S.label}>Valor do imóvel (R$)</label>{inp('valor_imovel','number','Ex: 780000')}</div>
