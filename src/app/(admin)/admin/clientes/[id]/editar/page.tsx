...
export default function EditarClientePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    createClient().from('clientes').select('*').eq('id', params.id).single()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    setError('')
    const { error: err } = await createClient().from('clientes').update(form).eq('id', params.id)
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <Link href={'/admin/clientes/' + params.id} style={{ fontSize: '13px', color: '#6b7280', textDecoration: 'none' }}>← Voltar</Link>
