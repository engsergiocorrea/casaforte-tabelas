...
export default async function ObraDetalhePage({ params }: { params: { id: string } }) {
  ...
  const calcPrazoAVencer = () => {
    const decorrido = calcPrazoDecorrido()
    if (decorrido === null || !obra.prazo_contratual_dias) return null
    return obra.prazo_contratual_dias - decorrido
  }

  const prazoDecorrido = calcPrazoDecorrido()
  const prazoAVencer = calcPrazoAVencer()
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      ...
      <div style={{ marginTop: '8px' }}>
        <Link href={'/admin/relatorios/novo?obra=' + obra.id}
          style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '10px 20px', background: '#1E1E1E', color: 'white', borderRadius: '8px', textDecoration: 'none', fontSize: '14px', fontWeight: '600' }}>
