...
export default function EditarObraPage({ params }: { params: { id: string } }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await createClient().from('obras').update(payload).eq('id', params.id)
    if (err) { setError(err.message); setSaving(false); return }
    router.push('/admin/obras/' + params.id)
  }
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <Link href={'/admin/obras/' + params.id} style={{ fontSize: '13px', color: '#6b7280', textDecoration: 'none' }}>← Voltar</Link>
