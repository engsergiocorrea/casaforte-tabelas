...
export default async function EngenheiroDetalhePage({ params }: { params: { id: string } }) {
  const supabase = await createClient()
  const { data: eng } = await supabase.from('engenheiros').select('*').eq('id', params.id).single()
  if (!eng) notFound()
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <Link href={'/admin/engenheiros/' + eng.id + '/editar'}
