...
const styles = StyleSheet.create({
  ...
  tableHeader: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#1e3a5f', paddingVertical: 6 },
...
const PDFDoc = ({ empreendimento, unidades }) => (
  <Document title={`Tabela - ${empreendimento.nome}`}>
    <Page size="A4" orientation="landscape" style={styles.page}>
      <View style={styles.header}>
        <Text style={styles.title}>{empreendimento.nome}</Text>
        <Text style={styles.subtitle}>{empreendimento.cidade}, {empreendimento.estado} · {new Date().toLocaleDateString('pt-BR')}</Text>
      </View>
      <View>
        ...
        {unidades.filter(u => u.status !== 'bloqueada' && u.status !== 'indisponivel').map(u => (
          <View key={u.id} style={styles.tableRow}>
            ...
            <Text style={styles.cell}>{u.area_total ? `${u.area_total}m²` : ''}</Text>
            <Text style={styles.cell}>{u.quartos ?? ''}</Text>
            <Text style={styles.cellBold}>{u.valor_imovel ? `R$ ${Number(u.valor_imovel).toLocaleString('pt-BR')}` : ''}</Text>
            <Text style={styles.cell}>{u.valor_sinal ? `R$ ${Number(u.valor_sinal).toLocaleString('pt-BR')}` : ''}</Text>
            <Text style={styles.cell}>{u.quantidade_parcelas && u.valor_parcela ? `${u.quantidade_parcelas}x R$ ${Number(u.valor_parcela).toLocaleString('pt-BR')}` : ''}</Text>
            <Text style={styles.cell}>{u.status ?? ''}</Text>
          </View>
        ))}
      </View>
      <Text style={styles.footer}>Os valores podem sofrer alteração sem aviso prévio. Casa Forte Construtora e Incorporadora.</Text>
    </Page>
  </Document>
)
...
export async function GET(request, { params }) {
  ...
  const { data: empreendimento } = await supabase.from('empreendimentos').select('*').eq('slug', params.slug).eq('ativo_publico', true).single()
  if (!empreendimento) return new NextResponse('Não encontrado', { status: 404 })
  const { data: unidades } = await supabase.from('unidades').select('*').eq('empreendimento_id', empreendimento.id).order('pavimento').order('unidade')
  try {
    const buffer = await renderToBuffer(<PDFDoc empreendimento={empreendimento} unidades={unidades ?? []} />)
